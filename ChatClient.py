...
def parse_cmds(data):
    ...
    if cmd == 'onlineSet':
        ...
        for u in data['nicks']:
            users = users + u + ','
        users = users[:-1]
        return '欢迎加入聊天室！请保证您已经阅读并同意了服务协议。\n如果您所在的聊天室没有在线的用户，' \
               '可以尝试加入聊天室 ?lounge\n在线的用户: ' + users
    if cmd == 'info':
        return "信息：" + data['text']
    if cmd == 'warn':
...
class ChatClientSetup:
    def __init__(self, root=None):
        ...
        Label(frame, text='用户名(可以#上密码)').grid(row=1, column=1)
        Entry(frame, textvariable=self.setup_user).grid(row=1, column=2)
        Label(frame, text='密码(可以为空)').grid(row=2, column=1)
...
class ChatClient:
    def __init__(self, root=None):
        self.api = 'ws://chat.henrize.kim:6060/'
        self.settings = Settings()
